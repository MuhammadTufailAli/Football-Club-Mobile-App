...
const TrainingReport = ({navigation}) => {
  ...
  const getTodayEvaluation = async () => {
    try {
      ...
      try {
        const result = await axios.get(
          `${port.herokuPort}/dailyrecords/GetDailyRecordsOfPlayer/${userdetails?.refOfPlayer}`,
        );
        ...
        try {
          const result = await axios.get(
            `${port.herokuPort}/dailyrecords/GetHighestScoreOfPlayer/${userdetails?.refOfPlayer}`,
          );
